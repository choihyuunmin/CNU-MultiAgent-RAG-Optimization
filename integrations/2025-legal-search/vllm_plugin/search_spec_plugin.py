...
def register():
    ...
    order = os.environ.get('SPEC_NGRAM_ORDER', 'earliest')
    ds_path = os.environ.get('SPEC_DATASTORE')
    stats_path = os.environ.get('SPEC_STATS')
    mode = os.environ.get('SPEC_MODE', 'ctx_then_ds')
    if mode not in ('ctx_then_ds', 'longest_any'):
        raise ValueError('SPEC_MODE must be ctx_then_ds or longest_any')
    if order not in ('earliest', 'latest'):
        raise ValueError('SPEC_NGRAM_ORDER must be earliest or latest')
