"""Bound model submissions while giving already-progressing workflows a turn.

The application owns its end-to-end deadline. Put this boundary OUTSIDE a
per-model execution timeout, so local dispatch waiting is not mistaken for an
inference failure. Queuing still counts toward the user's total response time.
No model input, generation option, tool call or output is rewritten here.
"""
