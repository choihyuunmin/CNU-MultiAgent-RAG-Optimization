"""Verified overlap of independent structured model calls within one request.

An application often issues a deterministic call whose every input is already
known before an earlier, unrelated call finishes. This adapter lets the caller
start that later call at the earlier point and consume it at the original call
site only when the inputs match exactly; otherwise the original call runs
unchanged. Prompts, model options, retries and admission behavior are not
touched; only the start time of already-required work moves earlier. Prepared
work that is never consumed is cancelled when the request scope closes or when
it becomes stale.
"""
