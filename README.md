...
## 2026-09-16 추가: 호출 수를 유지한 worker 추측 디코딩 실측

동일한 검색 도구 입력의 GPU 예비 96건을 완료했고, 동시 1/4에서 호출 시간이
35.0%/37.6% 줄었다. 별도 8문항의 전체 검색 64건에서는 평균이
19.46→13.49초(C1, 30.6%), 20.84→16.15초(C4, 22.5%)로 줄었다.
모든 요청이 내부·외부 기능 오류 없이 완료됐고, 32쌍 모두 반환 근거와 단계별 호출 수가 같았다.
최종 답변 텍스트는 달랐으며 전문가 정확도 동등성은 미입증이다.

이는 고정 4토큰 n-gram 추측 디코딩의 **같은 설정의 격리 모델 비교**다.
새 적응형 정책이나 운영 성능의 검증으로 주장하지 않는다. 별도 32문항·동시 4/16/32·
3반복의 확대 예비 실험을 진행 중이다.

[기능·실측·한계](docs/WORKER_DRAFT_RESULTS_20260916.md) ·
[원자료와 그래프](experiments/results/draft-budget-20260916/README.md).
