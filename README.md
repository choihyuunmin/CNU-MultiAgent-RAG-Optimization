...
## 2026-09-16 추가: worker 추측 디코딩 확대 검증 완료

도구 호출 재생 **96건**, 전체 검색 **640건**(첫 확인 64건 + 별도 32문항 확대 576건),
자동 근거 평가 **100건**을 완료했다. 요청·호출 제한을 새로 추가하지 않고, 원래 모델이
4개 제안 토큰을 검증하는 고정 n-gram 디코딩을 같은 설정의 격리 모델끼리 비교했다.

| 동시 요청 | 평균 기본→4토큰 | 평균 단축 | p95 기본→4토큰 |
|---:|---:|---:|---:|
| 4 | 23.62→18.35초 | 22.30% | 32.68→25.02초 |
| 16 | 51.81→37.44초 | 27.74% | 69.10→48.62초 |
| 32 | 76.93→57.10초 | 25.77% | 114.94→77.23초 |

확대 576건 중 내부·외부 기능 오류 없는 완료는 **575건**이다. 개선 조건의 통신 오류
1건은 대체 응답이 반환됐어도 실패로 유지했다. C32 근거 ID 재현율은 **97.97%**로
기본 자체 반복 **99.84%**보다 낮다. 원천 법령 포함률과 자동 평가 점수는 유지됐지만
전문가 정확도 동등성은 미입증이다. **새 방법의 64~200개 본 실험에는 승격하지 않았다.**

고정-k의 기존 기술이며 새 적응형 정책이나 운영 성능의 검증으로 주장하지 않는다.
실험 프로세스를 종료했고 기존 모델 서비스 8개의 정상 상태를 확인했다. 코드 테스트
339개 통과. 통신 오류의 원인 추적을 보완했지만 해당 실패 원인의 해결을 주장하지 않는다.

[확대 실험 전체 결과·채택 판단](docs/WORKER_DRAFT_CONFIRMATION_20260916.md) ·
[기능·첫 예비 결과](docs/WORKER_DRAFT_RESULTS_20260916.md) ·
[원자료와 감사 기록](experiments/results/draft-budget-20260916/README.md).

## 2026-09-16 추가: 재정렬 인증·단계 실패 판정·모델 호출 대기 보완

재정렬 HTTP 401과 오류 안내문을 정상 완료로 세던 문제를 확인했다. 인증·점수 응답을
검사하고 준비·선택 단계 실패를 추적한다. 공통 모델 호출 모듈에는 실행 수 제한,
대기와 개별 실행 제한의 분리, 선택적인 후속 호출 우선순위와 취소 처리를 추가했다.

완료·회수한 비교 **2,264요청**, 자동 근거 평가 **200건**. 인증 보정은 C32에서 평균
34.20→29.12초(14.85%)로 줄었다. 후속 우선 16은 C64에서 3.03% 느려 채택하지 않았다.
후속 우선 32의 C100 평균은 88.97→81.82초였지만 95% 구간이 0을 포함해 속도 개선은
확정하지 않는다. C100 내부·외부 기능 오류 없는 완료는 양쪽 모두 100%였다.

C200의 3조건·2반복 1,200요청은 네트워크 복구 후 회수했다. 오류 없는 완료는 제한 없음
59.75%, 선착순 32는 98.00%, 후속 우선 32는 83.00%였다. 후속 우선은 평균도
146.08→156.02초로 느려 채택하지 않는다. 선착순의 안정성 이득은 호출 제한 없이 계산을
가속하려는 새 연구의 성능 결과로 사용하지 않는다. 운영 배포는 변경하지 않았다.

[구체적인 기능](docs/CONTINUATION_HARNESS_20260916.md) ·
[완료한 전체 결과](docs/CONTINUATION_RESULTS_20260916.md) ·
[이전 결과 정정](docs/STAGE_OUTCOME_CORRECTION_20260916.md).

## 2026-09-16: 참조 ID 축약·복원, 동시 요청 1~200 실측 완료

> **완료율 정정:** 아래 표는 당시의 응답 완료 판정이며 내부 모델 실패를 일부 누락했다.
> 단계 로그 재검사 결과, 200개 구간의 모델·준비 단계 오류 없는 완료는 두 조건 모두 **57.5%**다.
> 재정렬 인증 오류도 확인했다. [정정 문서](docs/STAGE_OUTCOME_CORRECTION_20260916.md)를 함께 확인한다.

기능 설명을 **참조 ID 축약·복원 어댑터**로 정리했다. 모델에는 긴 문서 ID 대신
`0`, `1` 같은 번호를 전달하고, 선택 후 원래 ID로 복원한다. 본문·제목·점수는 유지한다.

동시 요청 **1·2·4·8·16·20·32·50·100·150·200**, 기존/개선 각 2회,
총 **2,896요청**을 측정했다. 개선 조건은 낮은 worker reasoning과 공통 ID 어댑터의
결합이다. 당시 완료 판정 2,741건, 시간 초과 155건을 모두 보존했다.

| 동시 요청 | 평균 관측시간 기존→개선 | 응답 완료율 기존→개선(정정 전) |
|---:|---:|---:|
| 100 | 96.64→84.74초 (12.32% 단축) | 100%→100% |
| 150 | 148.37→132.63초 (10.61% 단축) | 79%→100% |
| 200 | 153.16→150.65초 (1.64% 단축) | 83.25%→93.75% |

시간 초과의 240초를 포함한 관측시간이다. **200개 구간의 단축률 95% 구간은
−1.41~4.57%로 0을 포함하며 p95는 두 조건 모두 240초였다.** 이 구간의 근거 ID 재현율은
79.25%(기존 자체 반복 87.39%)이고 자동 근거 평가도 반복 간 변동이 커,
안정적인 속도·품질 개선을 입증하지 못했다. 전문가 정확도 유지도 미입증이다.
32개 이상은 유한한 동시 제출 묶음이며 지속적인 200명 처리 용량 시험은 아니다.

사전 지정 자동 평가 236건과 200개 두 번째 반복의 사후 평가 130건을 구분해 공개한다.
코드 테스트 265개 통과, 실험 문항 짝·지정 동시성·동결 코드 검증 완료.
운영 배포와 모델 설정은 변경하지 않았다.

[최종 결과·한계](docs/REFERENCE_ID_SCALING_RESULTS_20260916.md) ·
[실험 계획](docs/REFERENCE_ID_SCALING_PROTOCOL_20260916.md) ·
[원자료·그래프·감사](experiments/results/reference-scaling-20260916/README.md) ·
[구체적인 기능 설명](docs/REFERENCE_HARNESS_20260915.md).

## 2026-09-15 추가: 실제 검색기 1,800요청과 참조 ID 축약·복원

소규모 대조에서 효과·검색 결과를 확인한 뒤, 후보가 많을 때만 짧은 참조 ID를 쓰는
어댑터로 100고유 문항 × 3조건 × 동시 요청 4/20/100 × 2반복을 완료했다.
평균 지연은 기존 대비 **17.6% / 11.2% / 7.6%**, p95는 **20.4% / 16.6% / 8.4%** 감소했다.
기존 low-reasoning 설정을 제외한 새 ID 기능의 추가 단축은 **9.9% / 11.5% / 7.5%**다.
전체 1,800건 정상 완료. 자동 근거 평가 288건도 완료했으나 전문가 정확도 비열등성은
입증하지 못했다. C100의 기존 응답 대비 근거 ID 재현율은 93.0%이며 기존 자체 반복도
91.4%였다. 운영 배포는 하지 않았다.

범용성 요구에 따라 도메인·프레임워크·실행 그래프와 분리한 **참조 ID 축약·복원 어댑터**도
구현했다. 모델에는 긴 문서 ID 대신 `0`, `1` 같은 번호를 전달하고, 선택이 끝나면
실제 ID로 복원한다. ID가 있는 필드를 지정하고, 다른 필드의 ID 언급·중복·복원 오류를 검사한다.
짧은 ID 기법 자체는 기존 기술이며 학술적 신규성을 단정하지 않는다.
실제 검색기 성능과 별도의 순차/병렬/반복 구조 검증을 구분한다.

공통 모듈은 예비 비교 144건 후 별도 합성 워크플로 216건을 세 구조에서 검증했다.
총 모델 호출 768회, 정답 집합 모두 일치, 복원 재시도 0회였다. 참조가 많은 단순
합성 과제이므로 실제 검색기의 개선율이나 모든 업무에서의 정확도로 일반화하지 않는다.
[구조 실험 자료](experiments/results/reference-20260915/README.md).

[실제 검색기 결과](docs/STRUCTURED_HARNESS_RESULTS_20260915.md) ·
[범용 설계·적용 범위](docs/REFERENCE_HARNESS_20260915.md) ·
[실측 자료와 그래프](experiments/results/structured-20260915/README.md) ·
[공통 모듈](src/cnu_rag_optimization/reference_harness.py).
...
## 2026-09-14: 결합 하네스 어댑터 — 200문항 실측(지연 개선·자동 품질 평가)
...
support 1.80→1.831, 구간이 0 포함)은 관측 평균에서 저하를 보이지 않았다.
자동 평가이며 전문가 정확도 유지나 비열등성을 입증한 것은 아니다. 이득의 원천은 worker 답변 전
