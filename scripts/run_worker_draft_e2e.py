...
async def main():
    ...
    p.add_argument('--exclude-case-file',type=Path,
                   help='JSON list of previously used case IDs, including warmups')
    p.add_argument('--judge-level',type=int,
                   help='preselect all reference-answer cases at this load for later automatic judging')
    args=p.parse_args()
    if (args.case_count < 1 or args.repeats < 1 or not args.levels
            or args.levels != sorted(set(args.levels)) or args.levels[0] < 1
            or args.levels[-1] > args.case_count):
        p.error('levels must be positive, sorted, unique and reachable with the case count')
    if args.judge_level is not None and args.judge_level not in args.levels:
        p.error('judge level must be one of the measured levels')
    os.umask(0o077)
    ...
    if args.exclude_case_file:
        excluded.update(json.loads(args.exclude_case_file.read_text()))
    ...
    if len(eligible)<args.case_count+1:raise ValueError('not enough held-out labelled cases')
    warmup=eligible[:1];cases=eligible[1:args.case_count+1]
    ...
    plan={'utc':datetime.now(timezone.utc).isoformat(),'kind':'held_out_end_to_end_worker_speculation',
          ...
          'levels':args.levels,'repeats':args.repeats,'budgets':[0,4],
          'expected_requests':args.case_count*len(args.levels)*args.repeats*2,
          ...
          'excluded_case_ids':sorted(excluded),'warmup_case_ids':[c['case_id'] for c in warmup],
    ...
    if args.judge_level is not None:
        judge_cases=[c['case_id'] for c in cases if c.get('reference_answer')]
        if not judge_cases:raise ValueError('no supplied reference answers for judging')
        (out/'judge-plan.json').write_text(json.dumps({
            'created_utc':datetime.now(timezone.utc).isoformat(),'level':args.judge_level,
            'repeats':list(range(args.repeats)),'cases':judge_cases,'arms':['baseline','improved'],
            'expected':len(judge_cases)*2*args.repeats,'judge':'microsoft/phi-4',
            'selection':'all reference-answer questions in frozen pilot, before timing',
            'scope':'automatic evidence assessment, not expert correctness'},indent=2)+'\n')
    ...
    try:
        ...
        async with httpx.AsyncClient(timeout=5,trust_env=False) as client:
            for repeat in range(args.repeats):
                order=[0,4] if repeat%2==0 else [4,0]
                for k in order:
