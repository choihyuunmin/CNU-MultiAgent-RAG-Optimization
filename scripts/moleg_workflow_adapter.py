...
from cnu_rag_optimization.reasoning import ReasoningPolicy, StreamProgress, monitor_stream


def load_adapter(path):
    data = json.loads(Path(path).read_text())
    allowed = {"mode", "reservations", "stage_hooks", "serving_meter", "reasoning_policies"}
    if set(data) - allowed:
        raise ValueError("unknown adapter configuration fields")
    adapter = WorkflowAdapter(mode=data.get("mode", "observe"))
    adapter.reasoning_policies = {name: ReasoningPolicy(**value)
        for name, value in data.get("reasoning_policies", {}).items()}
...
def install(adapter, estimates, stage_hooks):
    ...
    async def chat(**kwargs):
        ...
        policy = getattr(adapter, "reasoning_policies", {}).get(model)
        if policy:
            kwargs = policy.payload(kwargs)
    ...
    async def stream(**kwargs):
        ...
        last_usage = {}
        reservation = await meter.estimate(model, kwargs) if meter and model in meter.roles else estimates.get(model)
        source = adapter.stream(model, lambda: monitor_stream(original_stream(**kwargs), progress, policy),
                                reservation=reservation)
        try:
            ...
            try:
                ...
                adapter.record("reasoning_size", started, model=model, **progress.metrics())
