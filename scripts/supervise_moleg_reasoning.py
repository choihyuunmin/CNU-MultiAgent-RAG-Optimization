"""Supervise the frozen two-arm unrestricted reasoning experiment on the GPU host.

Only owned loopback apps are started/stopped; model processes are read-only.
"""
import argparse
import asyncio
...
def campaign_spec(root):
    ...
    if (plan['unique_questions']!=300 or plan['users']!=USERS or plan['repeats']!=2
            or plan['variants']!=['baseline','reasoning'] or plan['planned_primary_requests']!=7200):
        raise ValueError('unexpected frozen reasoning experiment')
    smoke=json.loads((root/'smoke-verified/summary.json').read_text())
    if not smoke['complete'] or len(smoke['trials'])!=2 or any(
            t['n']!=8 or t['success']!=8 or t.get('pipeline_success')!=8
            or t.get('traced_internal_error_requests', 0)>0
            for t in smoke['trials']):
        raise ValueError('smoke comparison did not pass')
    return dict(limit=300,start_repeat=0,repeats=2,seed=20260911)
...
async def execute(root):
    ...
    planned=spec['limit']*len(USERS)*2*(spec['repeats']-spec['start_repeat'])
    started=utc()
    ...
    write_json(root/'main-model-inventory-before.json',models)
    ...
    app_config=[dict(name=name,slots=slots,emission=emission,port=port,
        ...
        ('baseline',None,'immediate',28360,'baseline.json'),
        ('reasoning',None,'immediate',28361,'reasoning.json')]]
    write_json(root/'main-arms.json',app_config)
    ...
    async def campaign():
        await command([sys.executable,str(scripts/'manage_moleg_scaling_apps.py'),'start','--directory',str(apps),
            ...
            '--arms-manifest',str(root/'main-arms.json')])
        ...
        await command([sys.executable,str(scripts/'analyze_moleg_workflow.py'),'--directory',str(root/'validation'),
                       '--workflow-directory',str(apps),'--comparison','baseline=reasoning'])
    ...
    try:
        ...
        try:
            ...
            write_json(root/'model-inventory-after.json',final_models)
            state['serving_processes_unchanged']=final_models==models
        except Exception:
            state['serving_processes_unchanged']=False
        ...
        write_json(status,state)
        log.close()
    if state['status']=='completed_evaluation_not_released':
        from export_moleg_reasoning import export
        export(root,root/'publication')
...
def main():
    ...
    args=p.parse_args()
    if (args.directory/'campaign-status.json').exists():
        p.error('refuse to overwrite an existing experiment status')
