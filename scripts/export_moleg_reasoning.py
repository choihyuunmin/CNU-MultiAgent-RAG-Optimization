"""Audit a completed unrestricted reasoning campaign and export numeric/hash-only publication files.
...
def validate_requests(rows,plan,summary):
    ...
    if plan['unique_questions']!=300 or plan['planned_primary_requests']!=7200 or len(schedule)!=24:
        raise ValueError('not the expected300 protocol')
    if not summary['complete'] or len(summary['trials'])!=24 or len(rows)!=7200:
...
def export(root,destination):
    ...
    frozen=json.loads((root/'primary-frozen.json').read_text())
    ...
    if len(qa)!=plan['source_reference_questions']:raise ValueError('QA membership count mismatch')
    for users in [1,2,4,8,16,32]:
        data=read_rows(root/f'judge-u{users}/answer_judgments.jsonl')
        expected={(case_id,arm) for case_id in qa for arm in ['baseline','reasoning']}
        ...
        if js['repeat']!=0 or js['completed']!=len(data) or js['expected']!=len(expected):
        ...
        judge_rows.extend({k:v for k,v in r.items() if k in JUDGE_KEYS}|dict(users=users,repeat=0) for r in data)
    artifacts={
        'campaign-status.json':status,'summary.json':summary,'plan.json':plan,'primary-frozen.json':frozen,
        **judge_summaries,
    }
    for name in ['validation/manifest.json','validation/workflow-analysis.json','validation/resources.json',
                 'environment-lineage.json','data-health-preflight.json','supervised-data-preflight.json',
                 'preflight.json','baseline.json','reasoning.json']:
        artifacts[Path(name).name]=json.loads((root/name).read_text())
    for label in ['smoke-validation','smoke-verified']:
        artifacts[label+'-summary.json']=json.loads((root/label/'summary.json').read_text())
    row_artifacts={
