"""Attach verified call overlap to the search service's orchestrator calls.

The service issues two sequential orchestrator calls whose inputs are both
known at request start: classification (+decomposition) and search preparation.
Preparation does not depend on the classification output. With ``adapter`` set,
the preparation call is started at the moment the classification call is
issued and consumed at the original preparation call site only when the
messages, response format, temperature, role and thinking option are exactly
those the application would have sent; any difference runs the original call.
Prompts, models, options and the application's parsing stay unchanged.

Without ``adapter`` the attachment only records output hashes of both calls so
that decisions can be compared across arms. It never records prompts, questions
or outputs. Private application source stays outside this repository.
"""
...
def install(*, adapter=None, emit=lambda event: None):
    ...
    original_graph = runner.run_moleg_graph
    ...
    async def graph_entry(*args, **kwargs):
        try:
    ...
    runner.run_moleg_graph = graph_entry
    if getattr(controller, "run_moleg_graph", None) is original_graph:
        controller.run_moleg_graph = graph_entry
