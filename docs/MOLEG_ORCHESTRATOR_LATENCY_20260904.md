...
## 추가 확인 (2026-09-04): 의도분석·검색어 추출이 느린 이유 — CoT 아님, 디코드 병목

"의도분석(classify)·검색어 추출(preparation)이 오래 걸린다"는 관찰을 CoT 관점에서
직접 확인했다. 실제 프롬프트로 gemma@8000을 호출해 토큰 구성을 측정했다.

| 단계 | prompt 토큰(프리필) | 출력 토큰 | reasoning 토큰 | `<think>` | 프리필 | 디코드 | 총 |
|---|---:|---:|---:|:--:|---:|---:|---:|
| 의도분석(classify) | ~2,260 | 9 | 0 | 없음 | ~68ms | ~155ms | ~220ms |
| 검색어 추출(preparation) | ~3,560 | 108 | 0 | 없음 | ~80ms | ~2,100ms | ~2,190ms |

- **이 단계들에는 CoT가 없다.** 추론 토큰 0개, `<think>` 없음, `enable_thinking`
  on/off가 출력(108토큰)을 바꾸지 않는다.
- 프리필은 무시할 수준(~80ms, prefix-cache). **지연의 실체는 디코드**다. preparation은
  108개 출력 토큰을 디코드하는 데 ~2.1초가 걸린다.
- 원인은 오케스트레이터 모델(gemma-4-31B)의 **디코드 처리량이 낮은 것**이다. GPU가
  유휴 상태(running=0)일 때도 gemma는 약 53 tok/s에 그쳤다.

| 모델 | 디코드 처리량(유휴) |
|---|---:|
| orchestrator gemma-4-31B | **53 tok/s** |
| worker gpt-oss-20b | 239 tok/s (4.5배) |
| comparison gemma4-e4b | 191 tok/s (3.6배) |

### 측정한 지렛대: 추출 단계를 빠른 모델로

같은 preparation 프롬프트를 20문항에서 gemma-4-31B vs gpt-oss-20b로 실행했다.

| 모델 | 평균 | p50 | p95 | 출력 토큰 |
|---|---:|---:|---:|---:|
| gemma-4-31B | 3,262ms | 3,496ms | 4,581ms | 116 |
| gpt-oss-20b (reasoning_effort=low) | 964ms | 935ms | 1,484ms | 212 |

**70.5% 단축(3.4배).** 정확도(gemma 기준): country 19/20, 가드레일 20/20, 키워드
Jaccard 0.91/0.80. 단 transformed_query는 표현이 크게 달라(완전일치 1/20) 검색
순위 영향을 검색 단계 회귀로 확인한 뒤 적용해야 한다.

정리: 이 단계의 지렛대는 CoT 축소가 아니라 ① 출력 토큰 축소, ② 구조화 추출을 빠른
모델로 이전, ③ 오케스트레이터 서빙 최적화(fp8/TP)로 디코드 처리량 높이기다.
