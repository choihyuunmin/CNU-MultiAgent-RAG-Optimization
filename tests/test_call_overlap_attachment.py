...
def _install_fake_app(calls, gate=None):
    ...
    async def run_graph(request_id, user_prompt, *, route="law_search"):
        ...
        return None
    runner = module("core.query_loop.runner", run_graph=run_graph,
                    _detect_multi_country_in_query=lambda text: (["A", "B"], text) if " and " in text else None)
    controller = module("api.controller.generate_controller", run_graph=run_graph)
...
def _cleanup():
    ...
    sys.modules.pop("call_overlap_attachment", None)
...
def test_preparation_starts_at_classify_and_is_reused_exactly_once():
    ...
    try:
        ...
        attachment = importlib.import_module("call_overlap_attachment")
        ...
        async def run():
            gate.set()
            return await controller.run_graph("req-1", " question ")
        ...
        assert controller.run_graph is runner.run_graph
    finally:
        _cleanup()
...
def test_multi_turn_classify_does_not_speculate_and_other_routes_discard_on_close():
    ...
    try:
        ...
        async def run():
            ...
            assert adapter.pending == 0
            await controller.run_graph("req-4", "greeting", route="greeting")
            assert adapter.pending == 0
        asyncio.run(run())
...
def test_trace_only_mode_changes_nothing_but_records_hashes():
    ...
    try:
        ...
        asyncio.run(controller.run_graph("req-5", "question"))
