"""Dependency-trial supervisor: 20-question smoke (both arms), then 200 x {barrier, ready} in two position-balanced rounds at concurrency 4.
...
def main(folder, timeout, idle_timeout, idle_max_running, concurrency):
    ...
    try:
        ...
        state({"state": "running", "plan": "round1: barrier,ready; round2: ready,barrier"})
        rc = subprocess.call(KUBE + base + ["--questions", "/opt/cnu/questions.jsonl", "--output", "/results/main", "--order", "barrier,ready", "--order", "ready,barrier"] + endpoints)
