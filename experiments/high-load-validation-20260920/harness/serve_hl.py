"""High-load validation server: original / direct arms on the frozen search service.
...
For the load sweep only, the isolated instance's admission capacity is raised to the
configured value (as in the 2026-09-13/17 sweeps); models, engines, prompts and the
search path are unchanged.
"""
import argparse
import hashlib
import importlib
...
def main():
    ...
    install_dispatch(search, args.mode, emit=emit)
    ...
    emit({"event": "trial_config", "mode": args.mode, "source_verified": True,
          ...
          "dispatch": args.mode, "stage": "high-load-validation-20260920"})
    import uvicorn
